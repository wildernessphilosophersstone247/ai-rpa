...
| apps | `curl http://192.168.3.207:8080/api/apps` + `python ... aivane_repl.py --apps` | ✅ 列出 50+ 个 launcher 应用 | `apps` 命令返回与 curl 相同的列表 |
| launch | `python aivane_repl.py --launch aivane.apprepl` | ✅ 返回 `Launched: aivane.apprepl` | 也已对其它 launcher app 做过启动验证 |
| list | `python aivane_repl.py --wait 2 --list` | ✅ 能列出桌面和 `AIVane REPL` 页面 ARIA 树 | 界面刷新后可重复调用 |
| tap | `python aivane_repl.py --tap <refId>` | ✅ 已在真机上成功点按按钮/图标 | 示例：成功点按 `复制 BASE URL` 等元素 |
| input | `python aivane_repl.py --input 4 "smoke123"` | ✅ 在 `AIVane REPL` 输入框中成功输入 | 真机日志和界面状态已验证 |
| swipe | `python aivane_repl.py --swipe up` | ✅ `Swiped up (duration=300ms, distance=0.5)` | Android touch swipe 接口可用 |
| back | `python aivane_repl.py --back` | ✅ `Pressed back` | Android back 操作执行成功 |
| press home | `python aivane_repl.py --press home` | ✅ `Pressed: home` | 启动器已回到主屏 |
| screenshot | `python aivane_repl.py --screenshot` | ✅ 已成功保存截图文件 | 首次需在设备上完成 MediaProjection 授权 |
